...
-- 테이블 수정 ALTER TABLE 테이블명; 
	...
	-- 컬럼명 수정  	RENAME COLUMN 기존컬렴명 TO 변경할컬럼명;
	-- 컬럼 타입 수정 	MODIFY(기존컬럼명 자료형(용량));
	-- 컬럼 추가		ADD (새컬럼명 자료형(용량)); 
	-- 컬럼 삭제 		DROP COLUMN 기존칼럼ㅁ명; 
	-- 제약조건 추가 
	-- ADD CONSTRAINT 제약조건이름 PRIMARY KEY(현재 테이블의 컬럼명); 
	-- ADD CONSTRAINT 제약조건이름 FOREIGN KEY(현재 테이블의 컬럼명) REFERENCES 상위테이블명(컬럼명); 
	-- 제약조건 삭제 : DROIP CONSTRAINT 제약조건이름; 
...
-- 컬럼명 수정 
ALTER TABLE TBL_MY_USER RENAME COLUMN USER_NAME TO USER_NICKNAME; ALTER 
...
-- 컬럼 타입 수정 
ALTER TABLE TBL_MY_USER MODIFY(USER_NICKNAME VARCHAR(25)); 
